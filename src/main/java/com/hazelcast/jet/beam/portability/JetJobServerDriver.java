...
public class JetJobServerDriver implements Runnable {

    private static final Logger LOG = LoggerFactory.getLogger(JetJobServerDriver.class);
    ...
    private static void printUsage(CmdLineParser parser) {
        System.err.println(
                String.format("Usage: java %s arguments...", JetJobServerDriver.class.getSimpleName()));
        parser.printUsage(System.err);
        System.err.println();
    }

    private static JetJobServerDriver fromParams(String[] args) {
    ...
    private static JetJobServerDriver fromConfig(ServerConfiguration configuration) {
        ThreadFactory threadFactory = new ThreadFactoryBuilder().setNameFormat("jet-runner-job-server").setDaemon(true).build();
        ListeningExecutorService executor = MoreExecutors.listeningDecorator(Executors.newCachedThreadPool(threadFactory));
        ServerFactory jobServerFactory = ServerFactory.createWithPortSupplier(() -> configuration.port);
        ServerFactory artifactServerFactory = ServerFactory.createWithPortSupplier(() -> configuration.artifactPort);
        return create(configuration, executor, jobServerFactory, artifactServerFactory);
    }

    public static JetJobServerDriver create(
            ServerConfiguration configuration,
            ListeningExecutorService executor,
            ServerFactory jobServerFactory,
            ServerFactory artifactServerFactory) {
        return new JetJobServerDriver(configuration, executor, jobServerFactory, artifactServerFactory);
    }


    private JetJobServerDriver(ServerConfiguration configuration, ListeningExecutorService executor, ServerFactory jobServerFactory, ServerFactory artifactServerFactory) {
    ...
    private JobInvoker createJobInvoker() {
        return new JetJobInvoker(executor, configuration);
    }
}
