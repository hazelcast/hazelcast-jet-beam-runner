...
public class SplittableDoFnTest extends AbstractRunnerTest {
    ...
    private static class PairStringWithIndexToLengthBounded extends PairStringWithIndexToLengthBase {
    }

    @DoFn.UnboundedPerElement
    private static class PairStringWithIndexToLengthUnbounded extends PairStringWithIndexToLengthBase {
    }
    ...
    private void testPairWithIndexBasic(IsBounded bounded) {
        PCollection<KV<String, Integer>> res =
                pipeline.apply(Create.of("a", "bb", "ccccc"))
    ...
    private void testSideInput(IsBounded bounded) {
        PCollectionView<String> sideInput =
                pipeline.apply("side input", Create.of("foo")).apply(View.asSingleton());

        PCollection<String> res =
                pipeline.apply("input", Create.of(0, 1, 2))
    ...
    private static class SDFWithLifecycleBase extends DoFn<String, String> {
        ...
        private transient SDFWithLifecycleBase.State state;

        @GetInitialRestriction
        public OffsetRange getInitialRestriction(String value) {
            assertEquals(SDFWithLifecycleBase.State.OUTSIDE_BUNDLE, state);
            return new OffsetRange(0, 1);
        }
        ...
        public void setUp() {
            ...
            state = SDFWithLifecycleBase.State.OUTSIDE_BUNDLE;
        }

        @StartBundle
        public void startBundle() {
            assertEquals(SDFWithLifecycleBase.State.OUTSIDE_BUNDLE, state);
            state = SDFWithLifecycleBase.State.INSIDE_BUNDLE;
        }

        @ProcessElement
        public void processElement(ProcessContext c, OffsetRangeTracker tracker) {
            assertEquals(SDFWithLifecycleBase.State.INSIDE_BUNDLE, state);
        ...
        public void tearDown() {
            assertEquals(SDFWithLifecycleBase.State.OUTSIDE_BUNDLE, state);
            state = SDFWithLifecycleBase.State.TORN_DOWN;
        }
    }
